...
토스 [frontend-fundamentals](https://frontend-fundamentals.com/)는 좋은 코드의 기준을 **가독성 · 예측 가능성 · 응집도 · 결합도** 4가지로 정의한다. 대표 문서 4편의 예시를 그대로 가져와, 같은 형식에 우리 반응형 코드를 대입해보았다.
...
클린코드는 여전히 AI가 짜준 코드를 사람이 리뷰해야된다는 점에서 중요한 부분이라고 생각한다.
우리가 하나의 방향성을 설립하기 위해서 아주 중요한 기준점중에 하나라고 생각한다.
...
오픈하고 조금 여유가 생긴 우리 팀에서 꼭 해결하고 갔으면해서 팀원들에게 안건을 열심히 제출 중이다. 
결론이 나면 또 업데이트를 해보겠다..!
