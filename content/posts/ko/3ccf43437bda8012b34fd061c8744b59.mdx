...
범위를 넓히면 응답이 커지고, 커지면 잘리고, 잘리는 위치가 매번 달라지면 같은 요청에 매번 다른 결과가 나온다. 노드 하나로 시작하고, 뷰포트와 상태 조합을 한 번에 다 요청하지 않고, 잘렸으면 `get_metadata`로 지도를 받아 필요한 노드만 다시 부른다.


여기서 한 발 더 밀면 이런 질문이 된다. 애초에 물어보지 않아도 되는 건 물어보지 않아도 되지 않을까. 디자인 시스템은 특히 그렇다. 화면은 매번 새로 그리지만 디자인 시스템의 값은 이미 정해져 있다. 버튼의 높이, 모서리 반경, 색, 글자 크기. 숫자와 이름으로 박혀 있는 것들이다. 이미 정해진 값을 매번 다시 추론하게 할 이유는 없어 보인다. 실제로 그 구간을 덜어내려는 시도들이 있다.
